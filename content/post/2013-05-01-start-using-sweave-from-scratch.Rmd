...
[Sweave](http://jeromyanglim.blogspot.com.es/2010/02/getting-started-with-sweave-r-latex.html){target="_blank"} is nothing more, nothing less than one of the best ways R can connect with a text editor, in this case [LaTeX](https://en.wikibooks.org/wiki/LaTeX/Introduction){target="_blank"}. 
...
Second, you need to install R, from [here](https://cran.r-project.org/){target="_blank"}. Once you have installed it, you will need to install a LaTeX distribution (my preferred LaTeX editor is TeXworks, and I have MiKTeX in MS Windows).

- If you are a MS Windows user, you will find useful this one: [MiKTeX](https://miktex.org/download){target="_blank"}. 

- For Linux users, here can be [Kile](https://kile.sourceforge.io/){target="_blank"} (from KDE), [TeXworks](http://www.tug.org/texworks/){target="_blank"}, or other alternatives. First of all you should install LaTeX (e.g. TeX Live), and configure it with these [tips](https://help.ubuntu.com/community/LaTeX){target="_blank"}.

- For Mac users, you can follow this [link (valid for other OSes)](https://www.latex-project.org/get/){target="_blank"} which provides instructions to install TeXmaker. You will be able to install TeXworks from [here](http://www.tug.org/texworks/){target="_blank"}.
...
Let's write something in a new Sweave file!. To this, you must create a new text file (e.g. `myfirstsweave.txt`) , and change its extension to `.Rnw` (see [this](https://support.microsoft.com/es-es/products/windows?os=windows-10){target="_blank"} for more help for MSWin users):

 `myfirstsweave.Rnw`

This is a blank file, but you can paste the following [code](https://stackoverflow.com/questions/8082429/plot-a-heart-in-r){target="_blank"} to construct a template:
...
Maybe it's not the cleanest solution, but I copied the [source code of `texi2dvi`](http://ftp.gnu.org/gnu/texinfo/texi2dvi){target="_blank"} and created a script with it: (`sudo gedit` if you are running Ubuntu, `sudo kate` if Kubuntu).  
...
- This [Sweave manual](https://stat.ethz.ch/R-manual/R-devel/library/utils/doc/Sweave.pdf){target="_blank"}, updated in 2017 by Friedrich Leisch and the R-core Team.

- These [Sweave customization tips](../../../post/data/Sweave-custom-Ihaka.pdf){target="_blank"}, written by Ross Ihaka himself. 
